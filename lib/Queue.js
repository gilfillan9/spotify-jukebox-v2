...
var Queue = {
    ...
    addTrack: function (track, noUpdate) {
        track.uuid = uuid.v4();
    ...
    serialise: function () {
        return Queue.queue.map(function (track) {
            return [track.link.split(":")[2], track.uuid]
        });
    },
    ...
    load: function () {
        ...
        if (fs.existsSync("data/queue.json")) {
            fs.readFile("data/queue.json", function (err, text) {
                ...
                require('./Spotify').getTracks(data.map((elem) => "spotify:track:" + elem[0])).then(Queue.addTracks).then(function () {
